...
const getBooksInYear=async function(req,res){
    ...
    let publishingYear=await BookModel.find({year:year})
    res.send({publishingYear})
}

const getXINRBooks=async function(req,res){
    let INRBook=await BookModel.find({$or:[{prices:{indianPrice:"100 INR"},prices:{indianPrice:"200 INR"},prices:{indianPrice:"500 INR"}}]})
    res.send({msg:INRBook})
}

const getRandomBooks= async function(req,res){
let RandomBooks=await BookModel.find({$or:[{stockAvaialable:true},{totalPages:{$gt:500}}]})
res.send({msg:RandomBooks})
}

const getParticularBooks= async function(req, res){
      let anything=req.body
      let particular=await BookModel.find(anything)
